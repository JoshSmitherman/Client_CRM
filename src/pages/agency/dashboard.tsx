...
export function DashboardPage() {
  ...
  // The headline figures and the attention lists do not depend on the filters,
  // so they are fetched once and left alone while the table re-queries.
  const summary = useQuery(
    () => Promise.all([getDashboardStats(), getFilterOptions(), getAttentionLists()]),
    [],
  );
  const projects = useQuery(() => getProjects(parseFilters(new URLSearchParams(search)), 25), [
    search,
  ]);
        ...
        {([stats, options, attention]) => (
          <>
            ...
            <section
              ...
              className="mb-6 grid gap-4 lg:grid-cols-2 2xl:grid-cols-3"
            >
              <AttentionList
            ...
            <section aria-label="Projects">
              <Card>
                ...
                <QueryBoundary query={projects}>
                  {(rows) => <ProjectTable projects={rows} />}
                </QueryBoundary>
              </Card>
            </section>
          </>
