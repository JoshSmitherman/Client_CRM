...
export function ProjectOnboardingTab() {
  const { projectId } = useProjectWorkspace();
  const query = useQuery(() => getOnboarding(projectId), [projectId]);
        ...
        return (
          <div className="space-y-4">
                  ...
                  <li key={section.id}>
                    <Card>
                      ...
                      <CardBody className="space-y-4">
                        ...
                        {/* A summary of the answers, so a reviewer does not have
                            to open a separate screen to decide. */}
                                ...
                                <div key={f.key} className={f.wide ? 'sm:col-span-2' : undefined}>
                                  ...
                                  <dd className="text-[13px] whitespace-pre-wrap">
                                    {f.type === 'checkbox'
                                      ? 'Confirmed'
                                      : f.type === 'select'
                                        ? (f.options?.find((o) => o.value === responses[f.key])
                                            ?.label ?? String(responses[f.key]))
            ...
            <p className="text-[12px] text-[var(--text-muted)]">
              ...
              <Link to={`/portal/projects/${projectId}/onboarding`} className="underline">
                their portal
              </Link>
              .
            </p>
