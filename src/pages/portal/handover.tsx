...
async function load(id: string) {
  ...
  // Nothing is shown until the agency has actually delivered it. The SELECT
  // policy says the same thing; this keeps the screen honest either way.
  if (!handover || handover.status === 'draft' || handover.status === 'ready') {
    return { project, handover: null } as const;
  }
...
export function PortalHandoverPage() {
  useDocumentTitle('Handover');
  const { id = '' } = useParams();
  ...
  return (
    <QueryBoundary query={query}>
      {(data) => {
        ...
        const { project, handover, documents, acceptances } = data;
                    ...
                    <ul className="divide-y divide-[var(--border-subtle)]">
                      {documents.map((doc) => (
                        <li key={doc.id} className="flex items-start gap-3 px-5 py-3">
                          ...
                          {doc.file_id ? (
                            <DownloadButton fileId={doc.file_id} label={doc.title} />
                          ) : null}
                          {doc.external_url ? (
                            <a
              ...
              <div className="space-y-4">
                {accepted ? (
                  ...
                  <AcceptanceForm
                    ...
                    canAccept={canAcceptHandover(profile.role)}
