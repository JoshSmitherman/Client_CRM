...
import { formatCurrency, formatDate, formatFileSize, formatRelative, isOverdue } from '@/lib/format';
import { requireAgency } from '@/lib/auth';
import { getProject, getProjectOverview, getProjectProgress } from '@/lib/queries/projects';
...
export default async function ProjectOverviewPage({
  ...
  const [overview, progress] = await Promise.all([getProjectOverview(id), getProjectProgress(id)]);
      ...
      <div className="space-y-4">
            ...
            <CardBody className="space-y-1">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[13px] font-medium">
                  {(overview.subscription.maintenance_plans as unknown as { name: string } | null)
                    ?.name ?? 'Plan'}
                </span>
              ...
              <p className="text-[12px] text-[var(--text-muted)]">
                {formatCurrency(overview.subscription.price, overview.subscription.currency)} ·
            ...
            <EmptyState
              ...
              action={
                <Button variant="secondary" size="sm" asChild>
                  <Link href={`${base}/maintenance`}>Set one up</Link>
                </Button>
              }
            />
          )}
        ...
        <Card>
          ...
          {overview.files.length === 0 ? (
            <EmptyState icon={FolderOpen} title="No files yet" description="Nothing uploaded." />
          ) : (
            <ul className="divide-y divide-[var(--border-subtle)]">
