  ...
  return (
    <>
      <PageHeader
        ...
        description={client.trading_name ? `Trading as ${client.trading_name}` : undefined}
        breadcrumbs={[{ label: 'Clients', href: '/clients' }, { label: client.company_name }]}
        meta={
          <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4 lg:col-span-2">
          {/* Projects */}
          <Card>
            <CardHeader title="Projects" description={`${overview.projects.length} in total`} />
            {overview.projects.length === 0 ? (
              <EmptyState
                ...
                action={
                  <Button asChild>
                    <Link href={`/projects/new?client=${id}`}>New project</Link>
                  </Button>
                }
              />
            ) : (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Card>
              ...
              {overview.changeRequests.length === 0 ? (
                <EmptyState icon={FileText} title="None open" description="Nothing outstanding." />
              ) : (
                <ul className="divide-y divide-[var(--border-subtle)]">
                    ...
                    <li key={cr.id}>
                      ...
                      >
                        ...
                        <span className="mt-1 flex items-center gap-2">
                          ...
                          <span className="text-[12px] text-[var(--text-muted)]">{cr.reference}</span>
                        </span>
                      </Link>
                    </li>
                  ))}
                  ...
                  {overview.supportRequests.map((sr) => (
                    <li key={sr.id}>
                      <Link
                        ...
                        className="block px-5 py-3 transition-colors hover:bg-[var(--surface-hover)]"
                      >
                        <span className="block truncate text-[13px] font-medium">{sr.subject}</span>
                        <span className="mt-1 flex items-center gap-2">
        ...
        <div className="space-y-4">
            ...
            <CardBody className="space-y-3 text-[13px]">
              ...
              {client.email ? (
                <p className="flex items-start gap-2">
                  <Mail className="mt-0.5 h-4 w-4 shrink-0 text-[var(--text-muted)]" aria-hidden="true" />
              ...
              {client.phone ? (
                <p className="flex items-start gap-2">
                  <Phone className="mt-0.5 h-4 w-4 shrink-0 text-[var(--text-muted)]" aria-hidden="true" />
              ...
              {client.website ? (
                <p className="flex items-start gap-2">
                  <Globe className="mt-0.5 h-4 w-4 shrink-0 text-[var(--text-muted)]" aria-hidden="true" />
                  <a
                    href={client.website}
                    target="_blank"
              ...
              {manager ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[12px] text-[var(--text-muted)]">{manager.email}</p>
                  </div>
                </div>
              ) : (
          ...
          <Card>
            <CardHeader
              ...
              action={
                <Link
                  href="/maintenance?tab=subscriptions"
                  ...
                  return (
                    <li key={sub.id} className="px-5 py-3">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[13px] font-medium">{plan?.name ?? 'Plan'}</span>
          ...
          <Card>
            ...
            />
            {overview.contacts.users.length === 0 && overview.contacts.invitations.length === 0 ? (
              <EmptyState
                icon={Users}
