...
export default async function PortalProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [onboarding, comments, { data: tasks }, { data: milestones }, { data: plan }, { data: activity }, { data: pages }, { data: handover }] =
    await Promise.all([
      getOnboarding(id),
  ...
  const stage = project.lifecycle_stages as unknown as { label: string; colour: string } | null;
    ...
    <>
      ...
      <section aria-label="Your actions" className="mb-6 grid gap-3 sm:grid-cols-3">
        {onboarding.sections.length > 0 && onboarding.completion < 100 ? (
          ...
          >
            <ClipboardList className="mt-0.5 h-5 w-5 shrink-0 text-[var(--accent-text)]" aria-hidden="true" />
            <span>
        ...
        {pagesNeedingContent > 0 ? (
          ...
          >
            <FileText className="mt-0.5 h-5 w-5 shrink-0 text-[var(--text-secondary)]" aria-hidden="true" />
            <span>
              <span className="block text-[14px] font-semibold">Website content</span>
              <span className="block text-[12px] text-[var(--text-secondary)]">
                {pagesNeedingContent} page{pagesNeedingContent === 1 ? '' : 's'} need your words
              </span>
            </span>
          </Link>
        ) : null}
        ...
        {handover && (handover.status === 'delivered' || handover.status === 'accepted') ? (
          <Link
            href={`/portal/projects/${id}/handover`}
            className="flex items-start gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-4 transition-colors hover:bg-[var(--surface-hover)]"
          >
            <Rocket className="mt-0.5 h-5 w-5 shrink-0 text-[var(--text-secondary)]" aria-hidden="true" />
            <span>
              <span className="block text-[14px] font-semibold">Handover pack</span>
              <span className="block text-[12px] text-[var(--text-secondary)]">
                {handover.status === 'accepted' ? 'Accepted — documents available' : 'Ready for you to review'}
              </span>
            </span>
          </Link>
        ) : null}
        ...
        <Link
          href={`/portal/requests/new?project=${id}`}
          className="flex items-start gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-4 transition-colors hover:bg-[var(--surface-hover)]"
        >
          <Upload className="mt-0.5 h-5 w-5 shrink-0 text-[var(--text-secondary)]" aria-hidden="true" />
          <span>
        ...
        <div className="space-y-4 lg:col-span-2">
          <Card>
            ...
            <CardBody className="space-y-3">
              <ProgressBar value={project.completion_percentage} label="Overall" size="lg" />
          ...
          {plan?.scope || plan?.objectives ? (
            <Card>
              ...
              <CardBody className="space-y-4">
                {plan.objectives ? (
                  <div>
                    <h3 className="text-[12px] font-medium text-[var(--text-muted)]">Objectives</h3>
                    <p className="mt-0.5 text-[13px] whitespace-pre-wrap">{plan.objectives}</p>
                  </div>
                ) : null}
                {plan.scope ? (
                  <div>
                    <h3 className="text-[12px] font-medium text-[var(--text-muted)]">Scope</h3>
