        ...
        <CardBody className="space-y-3">
          ...
          {onboarding.needsChanges > 0 ? (
            <p className="text-[13px] text-[var(--warning-text)]">
              {onboarding.needsChanges} section{onboarding.needsChanges === 1 ? '' : 's'} need
              {onboarding.needsChanges === 1 ? 's' : ''} a small change — look for the amber labels
              below.
            </p>
          ) : null}
          {nextSection ? (
            <Link
              href={`/portal/projects/${id}/onboarding/${nextSection.key}`}
        ...
        <ul className="divide-y divide-[var(--border-subtle)]">
          {onboarding.sections.map((section) => {
            ...
            return (
              <li key={section.id}>
                <Link
                  href={`/portal/projects/${id}/onboarding/${section.key}`}
