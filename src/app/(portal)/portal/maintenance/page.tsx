...
export default async function PortalMaintenancePage() {
  ...
  const [{ data: subscription }, plans, requests, { data: settings }] = await Promise.all([
    supabase
      ...
      .select('*, maintenance_plans!inner ( id, name, description, included_services, response_time_hours )')
      .is('deleted_at', null)
      .order('renewal_date')
      .limit(1)
      .maybeSingle(),
          ...
          <>
            ...
            {canRequestChange ? (
              <div className="mt-4 max-w-xl">
                <PlanRequestForm subscriptionId={null} plans={plans} currentPlanId={null} />
              </div>
            ) : null}
          </>
        ) : null}
  ...
  const [allowance, usage, { data: openRequests }, { data: completedRequests }] = await Promise.all([
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <Card>
            ...
            <CardBody className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-3">
                <Detail
                  label="Cost"
                  value={`${formatCurrency(subscription.price, subscription.currency)} ${BILLING_FREQUENCY_LABELS[subscription.billing_cycle].toLowerCase()}`}
                />
          ...
          <Card>
            <CardHeader title="What we have done" description="Time recorded against your plan." />
            <UsageTable usage={usage as unknown as UsageRow[]} linkBase="/portal/requests" />
          </Card>
